...
in the sequence shown above, the second right most digit to the second left most digit of the number in the sequence.
...
function thirt(n) {
  ...
  do {
    step = num;
    num = (num+'').split('').reverse().map((el, i) => el * pattern[i % 6]).reduce((a, b) => a+b);
  } while (step !== num);
  return num;
}