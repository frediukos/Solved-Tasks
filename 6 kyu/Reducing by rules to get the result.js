...
ALGORITHMS   BASIC LANGUAGE FEATURES   FUNDAMENTALS   ARRAYS   CONTROL FLOW
 */

function reduceByRules(num, rules) {
  let out = rules[0](num[0], num[1]);
  for (let i = 2; i < num.length; i++) {
    out = rules[(i - 1) % rules.length](out, num[i]);
  }
  return out;
}