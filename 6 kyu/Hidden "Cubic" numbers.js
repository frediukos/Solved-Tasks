...
if "cubic" numbers number1, number2, ... were found. The numbers in the output are to be in the order in which
they are encountered in the input string.
...
and the following is "234". If a packet of at most three digits has been taken, whether or not "cubic", it's over for that packet.

When a continuous string of digits exceeds 3, the string is split into groups of 3 from the left.
The last grouping could have 3, 2 or 1 digits. e.g "24172410" becomes 3 strings comprising "241", "724" and "10"
...
function isSumOfCubes(s){
  ...
  let out = arr.map(el => el.split('').map(el => el ** 3).reduce((a, b) => a + b));
  out = out.filter((el, i) => el === arr[i]*1);
  return out.length === 0
    ? 'Unlucky'
    : `${out.join(' ')} ${out.reduce((a, b) => a + b)} Lucky`;
}