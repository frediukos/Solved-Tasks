...
That means: if he already added the amount of flour for 2.8 cakes, he needs to add at least the amount of flour
...
For simplicity we ignore all units and just concentrate on the numbers. E.g. 250g of flour is simply 250 (units)
of flour and 1 lb of sugar is also simply 1 (unit) of sugar.
...
function getMissingIngredients(recipe, added) {
  ...
  for (let key in recipe) {
    if (added[key]) {
      if (added[key] < max * recipe[key]) out[key] = max * recipe[key] - added[key];
    } else out[key] = max * recipe[key];
  }
  return out;
}